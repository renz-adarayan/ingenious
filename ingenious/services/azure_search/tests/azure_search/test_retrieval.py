...
async def test_search_lexical(config: SearchConfig) -> None:
    ...
    retr = AzureSearchRetriever(config, search_client=_DummyLexClient(), embedding_client=_DummyEmbeddings())
...
async def test_search_vector(config: SearchConfig) -> None:
    ...
    retr = AzureSearchRetriever(config, search_client=_DummyLexClient(), embedding_client=emb_client)
