...
def _settings(use_semantic: bool = True) -> IngeniousSettings:
    ...
    s.models = [
        ModelSettings(
            model="text-embedding-3-small", deployment="emb", api_key="K", base_url="https://oai"
        ),
        ModelSettings(model="gpt-4o", deployment="chat", api_key="K", base_url="https://oai"),
    ]
    s.azure_search_services = [
        AzureSearchSettings(
            service="svc", endpoint="https://search.example.net", key="SK", index_name="idx", use_semantic_ranking=use_semantic
        ),
    ]
    return s
...
@pytest.mark.asyncio
async def test_provider_retrieve_timeout_propagates(monkeypatch: pytest.MonkeyPatch) -> None:
    settings = _settings()
    pipeline = MagicMock()
...
@pytest.mark.asyncio
async def test_provider_retrieve_topk_zero_returns_empty(monkeypatch: pytest.MonkeyPatch) -> None:
