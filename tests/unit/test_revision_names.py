...
class TestValidateRevisionId:
    """Test cases for validate_revision_id function."""

    def test_validate_revision_id_valid_cases(self):
        """Test valid revision ID formats."""
        valid_ids = [
            "myproject",
            "test-workflow",
            "project123",
        ...
        for revision_id in valid_ids:
            assert validate_revision_id(revision_id), f"'{revision_id}' should be valid"

    def test_validate_revision_id_invalid_cases(self):
        """Test invalid revision ID formats."""
        invalid_ids = [
            "",  # empty
        ...
        for revision_id in invalid_ids:
            assert not validate_revision_id(revision_id), f"'{revision_id}' should be invalid"

    def test_validate_revision_id_length_boundaries(self):
        """Test length boundary conditions."""
        assert validate_revision_id("a")  # length 1 (minimum)
        assert validate_revision_id("a" * 50)  # length 50 (maximum)
        assert not validate_revision_id("")  # length 0 (too short)
        assert not validate_revision_id("a" * 51)  # length 51 (too long)

    def test_validate_revision_id_pattern_matching(self):
        """Test regex pattern matching rules."""
        # Valid: only lowercase letters, numbers, hyphens
        assert validate_revision_id("abc123")
        assert validate_revision_id("test-123")
        assert validate_revision_id("a-b-c-1-2-3")
        
        # Invalid: special characters
        assert not validate_revision_id("test_123")
        assert not validate_revision_id("Test-123")
        assert not validate_revision_id("test@123")

    @patch("ingenious.utils.revision_names.logger")
    def test_validate_revision_id_logging(self, mock_logger):
        """Test that function logs validation properly."""
        result = validate_revision_id("test-project")
        
        mock_logger.debug.assert_called_once()
        call_args = mock_logger.debug.call_args
        assert "Validated revision ID" in call_args[0][0]
        assert call_args[1]["revision_id"] == "test-project"
        assert call_args[1]["is_valid"] == result
...
class TestGenerateRevisionId:
    ...
    def test_generate_revision_id_without_user_input(self):
        ...
        pattern = r"^[a-z]+-[a-z]+-[a-f0-9]{8}$"
        assert re.match(pattern, result), f"Generated ID '{result}' doesn't match funny name pattern"
